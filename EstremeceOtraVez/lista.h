...
public:
    ...
    /*
	 ...
	 Lineal O(n - 1), donde n es la longitud de la lista, el -1 pq da una vuelta menos pq la pos 0 nunca la mueve
	 
	*/
    void estremece() {

		if (this->esVacia() || this->longitud() == 1) return; // si vacía o de tamañoo 1, vuelve

		Nodo* act = this->_ult; // nodo actual
		Nodo* actAnt = this->_ult; // nodo anterior al actual

		for (int i = this->longitud() - 1; i >= 1; i--) { // recorro la lista al revés pero el primero no pq nunca se cambia
			actAnt = actAnt->_ant; // el nodo anterior al actual pasa a ser su anterior

			if (i % 2 != 0 && act != _ult) { // si mi pos es impar y además no soy el último
				act->_ant->_sig = act->_sig; // el siguiente a mi anterior es mi siguiente
				act->_sig->_ant = act->_ant; // el anterior a mi siguiente es mi anterior

				act->_sig = _ult->_sig; // mi siguiente es el siguiente del último
				act->_ant = _ult; //  mi anterior es el último
				_ult->_sig = act; // el siguiente del último soy yo
				_ult = act; // yo soy el nuevo último
			}
			act = actAnt; // actual pasa a ser su anterior
		}   
 	}
