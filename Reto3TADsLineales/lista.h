...
public:
	...
	//************ inicio subprograma pedido en ejercicio 18 ************

	/**
	Inserta el elemento elem de acuerdo con la posición pos(operación
	pon_ppio) de forma que cuando pos es 0 se añade al principio de la lista,
	mientras que cuando es igual al número de elementos de la lista se insertará
	detrás del último(operación pon_final).Indica la complejidad de la
	operación.Si llamas a otros métodos, debes implementarlos también

	@param elem el elemento a insertar en la lista
	@param pos la posición donde se debe insertar el elemento

	@author Álvaro Corrochano López
	*/
	void insertarPorPosicion(const T& elem, int pos) {  // Complejidad lineal, O(n), donde n es la pos.

		if (this->esVacia()) { this->pon_final(elem); return; }; // Si está vacía, lo añado al final.

		if (pos == 0) { this->pon_ppio(elem); return; }; // si va a la primera pos, lo pongo el primero.
		if (pos == this->longitud()) { this->pon_final(elem); return; }; // si va al final, lo pongo el último.

		Nodo* n = _prim; // nodo en el qque estoy.
		Nodo* sig = _prim->_sig; // mi siguiente.

		for (int i = 0; i < pos - 1; i++) { // recorremos la lista hasta la posición pos -1 pq empiezo inicializado, por lo que yo me quedo en pos y sig en mi siguiente, para así insertar entre nosotros dos.
			n = sig; // n pasa a ser su siguiente.
			sig = n->_sig; // el siguiente pasa a ser su siguiente.
		}

		this->insertaElem(elem, n, sig); // insertamos el elemento entre el nodo en el que estamos y su siguiente.

	}

	//************ fin subprograma pedido en ejercicio 18 ************
