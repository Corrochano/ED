...
int singularesAux(const Arbin<int>& a, int antepasados, int& sing) {
	if (a.esVacio()) return 0;

	int descIz = singularesAux(a.hijoIz(), antepasados + a.raiz(), sing);
	int descDer = singularesAux(a.hijoDer(), antepasados + a.raiz(), sing);

	if (descIz + descDer == antepasados) sing++;

	return descIz + descDer + a.raiz();
}
