...
public:
	...
	COMPLEJIDAD: Determinar aquí, justificadamente, la complejidad de la operación cortaPorK
		La complejidad es lineal O(n) donde n es k, dado que las vueltas del bucle depende de k

	**/
	void cortaPorK(unsigned int k, Lista<T>& destino) {

		if (this->esVacia()) return; // si la lista está vacía, vuelvo

		if (k >= this->longitud()) return; // si la k es mayor o igual que la longitud, vuelvo

		Nodo* act = this->_prim; // nodo por el que voy
		Nodo* actSig = this->_prim; // su siguiente

		Nodo* dest = destino._prim; // nodo por el que voy añadiendo en la lista destino

		for (int i = 0; i <= k; i++) { // recorro la lista hasta k

			if (actSig != this->_ult) { // si mi siguiente no es el último
				actSig = actSig->_sig; // es su siguiente
			}

			if (i == 0) { // si es el primero
				if (act == _ult){ // si soy el último
					dest = act; // destino es igual a actual
					dest->_sig = NULL; // el siguiente de destino es null
					dest->_ant = NULL; // el anterior de destino es null
					destino._prim = dest; // el primero de la lista destino es dest
					destino._ult = dest; // el último de la lista destino es dest
					_numElems--; // actualizo mi número de elemenos
					destino._numElems++; // actualizo el número de elementos de destino
					_prim = NULL; // me quedo vacío
					_ult = NULL; // me quedo vacío
				}
				else { // si no soy el último
					act->_sig->_ant = NULL; // el anterior de mi siguiente es NULL
					dest = act; // destino es igual a actual
					dest->_sig = NULL; // el siguiente de destino es null
					dest->_ant = NULL; // el anterior de destino es null
					destino._prim = dest; // el primero del¡ la lista destino es dest
					destino._ult = dest; // el último de la lista destino es el dest
					_numElems--; // actualizo mi número de elemenos
					destino._numElems++; // actualizo el número de elementos de destino
					_prim = actSig; // el nuevo primero es mi siguiente
				}
			}
			else {
				_prim = act->_sig; // el nuevo primero es mi siguiente

				if (act != _ult) { // si act no es el último
					act->_sig->_ant = act->_ant; // el anterior de mi siguiente es mi anterior
				}
				else { // si es el último
					_ult = NULL; // el último es NULL
				}

				dest->_sig = act; // el siguiente del destino es el actual
				act->_sig = dest->_sig; // el siguiente del actual es el siguiente del destino
				act->_ant = dest; // el anterior del actual es el destino

				if (i == k) { // si soy la última vuelta
					destino._ult = act; //  el último del destino es el actual
					act->_sig = NULL; // su siguiente es NULL
				}

				dest = dest->_sig; // destino pasa a ser su siguiente

				_numElems--; // actualizo mi número de elementos
				destino._numElems++; // actualizo el número de elementos de la lista destino

			}

			act = actSig; // el actual pasa a ser su siguiente
		}
	}
