...
public:
	...
	//
	// para intentar conseguir O(1) puedes cambiar, si lo necesitas, el prototipo de la operación

	void concatena(const Lista<T>& lista) {
		
	/**
		**	Intento de solución con complejidad constante O(1), no funciona correctamente.	**
	*/
		
		if (!lista.esVacia()) { // si la lista dada por parámetro no está vacía.
			if (!this->esVacia()) { // si yo no estoy vacío.
				_ult->_sig = lista._prim; // el siguiente a mi último es el primero de la lista dada por parámetro.
				_ult->_sig->_ant = _ult; //  su anterior es mi último.
				_ult = lista._ult; // mi último es el último de la lista dada por parámetro.
				this->_numElems += lista._numElems; // actualizamos mi número de elementos.
			}
			else { // si estoy vacío.
				this->_prim = lista._prim; // el primero de la lista dada por parámetro es mi primero.
				this->_ult = lista._ult; // el último de la lista dada por parámetro es mi último.
				this->_numElems += lista._numElems; // actualizamos mi número de elementos.
			}
	...
	/**
		**	Solución lineal O(n) donde n es el tamaño de la lista pasada por parámetro.	**
	*/
		/*
		if (!lista.esVacia()) {  // Si la lista dada no está vacía.
			
			typename Lista<T>::ConstIterator it = lista.cbegin(); // iterador para la lista dada por parámetro.
			
			while (it != lista.cend()) { // si el iterador no ha llegado al final de la lista.
				this->pon_final(it.elem()); // pon al final el elemento al que apunta el iterador.
				it.next(); // aumenta el iterador.
			}
		}
		*/
	}
	...
	//
	// FIN EXTENSIÓN PARA RESOLVER EL EJERCICIO 17
	//
