...
unsigned int profundas(const Arbin<int>& a, unsigned int k, int prof) {
	...
	unsigned int iz = profundas(a.hijoIz(), k, prof + 1);
	unsigned int der = profundas(a.hijoDer(), k, prof + 1);
...
/**
  COMPLEJIDAD: Explica aquí justificadamente cuál es la complejidad de 
               esta función
			   Complejidad lineal O(n) dado que solo visitamos una vez cada nodo del árbol, haciendo una única llamada a cada hijo.
*/
unsigned int numero_hojas_mas_profundas_que(const Arbin<int>& a, unsigned int k) {
	return profundas(a, k, 1);
}
