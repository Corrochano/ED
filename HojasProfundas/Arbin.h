...
#include <fstream>  // necesario para la sobrecarga de <<
using namespace std;
...
class Arbin {
   public:	  
	  ...
	  // EXTENSIÓN DEL TAD ARBIN PARA VOLCAR UN ARBOL EN UN STREAM

	  /**
		 Escritura de un árbol en un stream.

		 @param os El stream en el que se va escribir el árbol
		 @param a El arbol a escribir
		 @return una referencia al stream en el que se ha escrito el árbol
		 */

	  friend ostream& operator<<(ostream& os, const Arbin& a) {
		  a.print(os, a._ra, 0);
		  return os;
	  }
   ...
   private:
	 ...
	 // MÉTODOS PRIVADOS USADOS PARA VOLCAR UN ARBOL EN UN STREAM

	/** Identa una línea de texto en un stream.

			@param os, el stream sobre el que realizar el indentado
			@param id, número de espacios en blanco a escribir
	*/
	 static void identa(ostream& os, int id) {
		 for (int i = 0; i < id; i++)
			 os << " ";
	 }

	 /** Realiza la impresión de una estructura de nodos.

	 @param os, el stream sobre el que realizar la impesión
	 @param n, puntero a la raíz de la estructura a imprimir
	 @param id, identado a aplicar (número de espacios en blanco a escribir
	 al comienzo de la línea)
	 */
	 static void print(ostream& os, Nodo* n, int id) {
		 if (n == NULL) { identa(os, id); os << "#" << endl; }
		 else if (n->_iz == NULL && n->_dr == NULL) { identa(os, id); os << n->_elem << endl; }
		 else {
			 identa(os, id); os << n->_elem << "(" << endl;
			 print(os, n->_iz, id + 2);
			 print(os, n->_dr, id + 2);
			 identa(os, id); os << ")" << endl;
		 }
	 }
