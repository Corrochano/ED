...
public:
	...
	*/
	...
	/**
		Reordena la lista poniendo primero último segundo penúltimo...
	*/
	void reordenar() {
		if (this->esVacia() || this->longitud() == 1) return; // si está vacía o solo tiene un elemento, vuelve

		Nodo* iz = this->_prim; // Nodo que va por la izquierda
		Nodo* izSig = this->_prim; // el siguiente del que va por la izquierda
		Nodo* der = this->_ult; // nodo que va por la derecha

		while (iz != der && iz->_sig != der) { // si no estamos en el mismo o no ha adelantado iz a der
			izSig = izSig->_sig; // el izSig pasa a ser su siguiente
			
			der->_ant->_sig = der->_sig; // el anterior al último es el null del final
			_ult = der->_ant; // el último es mi anterior

			iz->_sig = der; // el siguiente del de la izquierda es el último
			izSig->_ant = der; // el anterior del siguiente al de la izquierda es el último
			der->_sig = izSig; // el siguiente al de la derecha es el siguiente al izquierdo
			der->_ant = iz; // el anterior al de la derecha es el izquierdo

			der = _ult; // actualizamos derecha
			iz = izSig; // actualizamos izquierda
		}
	}
