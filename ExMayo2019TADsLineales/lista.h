...
public:
	...
	*/
    void estremece() {
		...
		if (this->esVacia() || this->longitud() == 1) return; // si la lista está vacía o tiene solo 1 elemento, vuelvo

		Nodo* act = this->_ult; // nodo por el que voy
		Nodo* ant = this->_ult; // mi anterior

		ant = ant->_ant; // el anterior va una posición detrás mío

		for (int i = this->longitud() - 1; i >= 0; i--) { // recorro de delante hacia atrás
			
			if (i % 2 != 0) { // si es impar

				if (_ult != act) { // si el actual no es el último
					ant->_sig = act->_sig; // El siguiente a mi anterior es mi siguiente
					act->_sig->_ant = act->_ant; // el anterior a mi siguiente es mi anterior

					act->_sig = _ult->_sig; // mi siguiente es el siguiente al último
					_ult->_sig = act; // el siguiente al último soy yo
			...
			act = ant; // el actual es el anterior

			if (ant != _prim) { // si el anterior no es el primero
				ant = ant->_ant; // va uno para atrás
			}
