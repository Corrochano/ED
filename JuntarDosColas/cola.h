...
public:
	...
	*/
	...
	void entremezclar(Cola<int>& cola2, int tam1, int tam2) {
		if (tam1 == 0) this->_prim = cola2._prim; this->_ult = cola2._ult; return;
		if (tam2 == 0) return;
		
		int tamTotal = tam1 + tam2;
		Nodo* current = this->_prim;
		Nodo* next;

		for (int i = 0; i < tamTotal && tam1 != -1 && tam2 != 0; i++) {
			if (tam1 != 0) {
				next = current->_sig;
				tam1--;
			}
			else {
				cola2._prim->_sig = current->_sig;
				current->_sig = cola2._prim;
				cola2._ult->_sig = this->_ult->_sig;
				this->_ult = cola2._ult;
				tam1--;
			}

			if (i % 2 != 0 && tam2 != 0) {
				current->_sig = cola2._prim;
				cola2._prim = _prim->_sig;
				current->_sig->_sig = next;
				tam2--;
			}
			current = next;
		}
