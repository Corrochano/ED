...
public:
	...
	*/
	...
	void transposicion() { // Complejidad Lineal O(n) donde n es la longitud de la lista
		if (this->longitud() < 3) return; // si el tamaño es menor que 3, me salgo
		
		Nodo* peq = this->_prim; // último nodo con el elemento más pequeño
		Nodo* act = this->_prim; // nodo actual de la búsqueda del while

		while (act != NULL) { // si act no es null
			
			if (act->_elem <= peq->_elem) { // si el elemento de act es menor o igual que el de peq
				peq = act; // peq se actualiza a act
			}

			act = act->_sig; // act pasa a ser su siguiente
		}

		if (peq == _prim || peq == _ult) return; // si peq es el primero o el último, no hace nada, vuelve

		if (_prim != peq->_ant) { // si mi anterior no es el primero
			peq->_ant->_ant->_sig = peq->_sig; // el siguiente del anterior de mi anterior es mi siguiente
		}

		if (_ult != peq->_sig) { // si mi siguiente no es el último
			peq->_sig->_sig->_ant = peq->_ant; // el anterior del siguiente de mi siguiente es mi anterior
		}

		act = peq->_sig->_sig; // act pasa a ser el siguiente de peq

		peq->_sig->_sig = peq; // el siguiente a mi siguiente soy yo

		peq->_ant->_sig = act; // el siguiente a mi anterior es el siguiente de mi siguiente

		act = peq->_ant->_ant; // el actual es el anterior a mi anterior

		peq->_ant->_ant = peq; // el anterior a mi anterior soy yo

		peq->_sig->_ant = act; // el anterior a mi siguiente es el anterior del anterior

		act = peq->_sig; // el actual es mi siguiente

		peq->_sig = peq->_ant; // mi siguiente es mi anterior

		peq->_ant = act; // mi anterior es mi siguiente

		if (peq->_ant == _ult && peq->_sig == _prim) { // si mi anterior es el primero y mi siguiente es el último
			_ult = peq->_sig; //  el nuevo último es el siguiente
			_prim = peq->_ant; // el nuevo primero es mi anterior
		}
		else if (peq->_ant == _ult){ // si solo mi anterior es el último
			_ult = peq->_sig; // el nuevo último es mi siguiente
		}
		else if (peq->_sig == _prim) {// si solo mi siguiente es el primero
			_prim = peq->_ant; // el nuevo primero es mi anterior
		}
	}
