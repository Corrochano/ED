...
public: 	
    /*
	 Operación 'invertir' a implementar
	 Aparte de este método, puedes implementar otros métodos (privados) auxiliares
	 que consideres oportunos. 
	*/
	
	/* COMPLEJIDAD:  
	 INDICA Y JUSTIFICA AQUÍ LA COMPLEJIDAD DE TU IMPLEMENTACIÓN:
	 
	 */

	 /*
		aux = this->_prim; // aux = 1
		_prim = this->_prim->_sig; // prim = 2
		_prim->_ant = aux->_ant; // null <- 2
		aux->_sig = _ult->_sig; // 1 -> null
		aux->_ant = _ult; // 4 <- 1
		this->_ult->_sig = aux; // 4 -> 1
		this->_ult = aux; // ult = 1
	*/
	...
	*/
    void invertir() { // Lineal O(n), donde n = num elems de la lista.
		Nodo* aux;

		_ult = _prim; // _ult = 1

		for (int i = 0; i < longitud(); i++) {
			aux = this->_prim->_sig; // aux = 2;
			this->_prim->_sig = _prim->_ant; // 1 -> null
			this->_prim->_ant = aux; // 2 <- 1
			if (i != longitud() - 1) { // No quiero última vuelta
				_prim = aux; // prim = 2 // si lo hiciese en la última pasaría esto: prim = null pq 4 -> null // en la penúltima ult = 4 pq es 3 -> 4
			}
		}
	...
	/**
	 Operación añadida especificamente para el examen.
	 Visualiza la lista por pantalla de principio a fin.
	...
	*/
	...
	/**
	 Operación añadida especificamente para el examen.
	 Visualiza la lista por pantalla del final al principio.
