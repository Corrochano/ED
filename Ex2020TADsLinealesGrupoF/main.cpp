 ...
 */
  ...
  */
  ...
  /*@ <answer>
   *
   * Nombre y apellidos: _Álvaro_Corrochano_López_
   *
   *@ </answer> */
 ...
 * --------------------------------------------------------------------
 *
 * Puedes anadir los métodos privados auxiliares que quieras. Para eso
...
void ListLinkedDouble<T>::concat_reverse(ListLinkedDouble& other) {
    ...
    if (other.empty()) return; // si la otra está vacía, me quedo igual

    Node* last = other.head->prev; // nodo al que empieza en el último de la otra lista
    Node* lastPrev = other.head->prev; // nodo que va en el anterior al otro

     do {
        if (lastPrev != other.head) { // si no es fantasma
            lastPrev = lastPrev->prev; // va uno para atrás
        }

        this->head->prev->next = last; // el siguiente a mi último es su último
        this->head->prev = last; // mi último es su último
        last->next = head; // el siguiente a su último es mi fantasma

        last = lastPrev; // last va hacia atrás
     } while (lastPrev != other.head); // mientras el anterior a mí no sea el fantasma

     num_elems += other.num_elems; // actualizo el número de elementos que tengo
}
