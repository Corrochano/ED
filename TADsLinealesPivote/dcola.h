...
public:
	...
	**/
	void particion(int pivote, int n) {
		Nodo* current = this->_fantasma->_sig; // yo
		Nodo* sig; // mi siguiente
		Nodo* prev; // mi anterior

		for (int i = 0; i < n; i++) { // vemos una vez cada elemento
			sig = current->_sig; // Me guardo el siguiente
			if (current->_elem > pivote) { // Si es mayor que el pivote
				prev = current->_ant; // me guardo mi anterior
				current->_ant = _fantasma->_ant; // el anterior a mí es el último
				_fantasma->_ant->_sig = current; // el siguiente al último soy yo 
				_fantasma->_ant = current; // el anterior al fantasma soy yo
				current->_sig = _fantasma; // mi siguiente es el fantasma
				sig->_ant = prev; // el anterior a mi antiguo siguiente es mi antiguo anterior
				prev->_sig = sig; // el siguiente a mi antiguo anterior es mi antiguo siguiente
			}
			current = sig; // paso al siguiente
		}
	}
